...
tooltip <- list(
  ...
  highcharter::hc_title(
    text = paste("Evolução da balança comercial anual (2014-2026) do", 
...
exp_br_produtos2026 <- 
  ncm_exportacao_agrupado |> 
  dplyr::filter(ano == max(ano) & no_cuci_grupo %in% lbls & uf == input$SelecaoUF
                # uf == "Maranhão"
                ) |> 
...
# print(dplyr::glimpse(exp_br_produtos2026))
#  
...
######### TABELA ########################################
 ...
 titulo <- htmltools::tags$caption(
  ...
  htmltools::tags$p("Os principais produtos exportados em ", max(ncm_exportacao_reativo()$ano, na.rm = TRUE))
)
  legenda <- htmltools::tags$caption(
    style = 'caption-side: bottom; text-align: left;', # Legenda à esquerda embaixo
    htmltools::tags$p("Fonte: Secretaria de Comércio Exterior - SECEX (2026)")
  )
...
exp_br_produtos2026 |>
  DT::datatable(
    ...
    colnames = c("Mês", colnames(exp_br_produtos2026)[-1]), # Define os nomes das colunas
    caption = cabecalho,
    escape = FALSE # Permite HTML na tabela (para o rodapé estilizado)
  ) |>
  DT::formatCurrency(
    columns = colnames(exp_br_produtos2026)[-1], # Formata todas as colunas menos a primeira
    currency = "", mark = ".", dec.mark = ",", digits = 2
  ) |>
  DT::formatStyle(
    columns = 0:ncol(exp_br_produtos2026), # Aplica o estilo a todas as colunas
    target = "row",
    backgroundColor = DT::styleEqual(seq(1, nrow(exp_br_produtos2026), 2), "#f2f2f2") # Cor de fundo alternada
  ) |>
  DT::formatStyle(
    columns = 0:ncol(exp_br_produtos2026), # Aplica o estilo a todas as colunas
    target = "row",
    fontWeight = DT::styleEqual(nrow(exp_br_produtos2026), "bold") # Última linha em negrito
  )  
  })
...
# print(exp_pais_2014_2026)
# cat("Nome das colunas\n")
# print(colnames(exp_pais_2014_2026))
...
highcharter::hchart(
  ...
  highcharter::hc_title(
    text = paste("Os Principais Destinos das Exportações do ", 
                 input$SelecaoUF, " entre 2014-2026, valores ",
                 extrair_texto_entre_parenteses(input$variavelY)),
    style = list(fontSize = "20px", fontWeight = "bold"))  
...
lbls <- 
  ...
  dplyr::filter(ano == max(ano, na.rm = TRUE) & uf == input$SelecaoUF
                  # uf == "Maranhão"
                ) |> 
  dplyr::group_by(no_cuci_grupo) |> 
  dplyr::summarise(
...
imp_br_produtos2026 <- 
  ncm_importacao_agrupado |> 
  dplyr::filter(ano == max(ano) & no_cuci_grupo %in% lbls & 
                uf == input$SelecaoUF
                # uf == "Maranhão"
                ) |> 
  dplyr::group_by(nome_mes, no_cuci_grupo) |> 
...
# print(dplyr::glimpse(Imp_br_produtos2026))
#  
# adicionando linha de total
imp_br_produtos2026 <-
imp_br_produtos2026 |> 
...
######### TABELA ########################################
  ...
  titulo <- htmltools::tags$caption(
  style = 'caption-side: top; text-align: center; font-size: 20px; font-weight: bold;',
  htmltools::tags$p("Os principais produtos importados em ", max(ncm_importacao_reativo()$ano, na.rm = TRUE))
)
  legenda <- htmltools::tags$caption(
    style = 'caption-side: bottom; text-align: left;', # Legenda à esquerda embaixo
    htmltools::tags$p("Fonte: Secretaria de Comércio Exterior - SECEX (2026)")
...
imp_br_produtos2026 |>
  DT::datatable(
    ...
    rownames = FALSE, # Remove os números das linhas
    colnames = c("Mês", colnames(imp_br_produtos2026)[-1]), # Define os nomes das colunas
    caption = cabecalho,
    escape = FALSE # Permite HTML na tabela (para o rodapé estilizado)
  ) |>
  DT::formatCurrency(
    columns = colnames(imp_br_produtos2026)[-1], # Formata todas as colunas menos a primeira
    currency = "", mark = ".", dec.mark = ",", digits = 2
  ) |>
  DT::formatStyle(
    columns = 0:ncol(imp_br_produtos2026), # Aplica o estilo a todas as colunas
    target = "row",
    backgroundColor = DT::styleEqual(seq(1, nrow(imp_br_produtos2026), 2), "#f2f2f2") # Cor de fundo alternada
  ) |>
  DT::formatStyle(
    columns = 0:ncol(imp_br_produtos2026), # Aplica o estilo a todas as colunas
    target = "row",
    fontWeight = DT::styleEqual(nrow(imp_br_produtos2026), "bold") # Última linha em negrito
  )  
  })
...
# cat("Print depois do slice_max\n")
# print(imp_pais_2014_2026)
# cat("Nome das colunas\n")
# print(colnames(imp_pais_2014_2026))
